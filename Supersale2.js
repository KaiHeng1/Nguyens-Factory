...
function addToCart() {
    if (selectedColor && selectedSize) {
        ...
        cartItem.dataset.itemId = uniqueId; // Setting a dataset attribute for the cart item ID
        ...
        cartItem.querySelector('.cancel').addEventListener('click', () => {
            const key = cartItem.dataset.itemId; // Retrieve the unique ID when cancel is clicked
            LocalCart.removeItemFromCart(key);
            cartWrapper.removeChild(cartItem); // Remove the cart item from the UI
        });
...
function generateUniqueId() {
    // Generate a unique ID based on current timestamp or other unique logic
    return 'item_' + new Date().getTime(); // Example: Generating a timestamp-based unique ID
}
